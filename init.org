...
** Typescript
   ...
   LSP mode is disabled because it is slow as hell for big typescript
   projects. Use tide instead.
...
* Mails
  ...
  I use notmuch + offlineimap for reading my mails.

** Offlineimap

   On Mac, download it with: ~brew install offlineimap~.

   Here is an example offlineimap config for GMail to put in ~~/.offlineimaprc~:
   ...
   For having a list of the Gmail folders, you can comment the
   ~folderfilter~ line and run ~offlineimap --dry-run~.

   In the end, run on Mac OS ~brew services start offlineimap~ to get it to launch at restart.

** Notmuch

   Install and configure notmuch with:
   ...
   #+BEGIN_SRC sh :tangle no
     ...
     # Performs the first index
     notmuch new
   #+END_SRC

   In the end, I use ~notmuch.el~:

   #+BEGIN_SRC emacs-lisp
     (fset 'my-notmuch-direct-archive
	(kmacro-lambda-form [?- ?u ?n ?r ?e ?a ?d ?  ?- ?i ?n ?b ?o ?x return ?\C-x ?\( ?- ?u ?n ?r ?e ?a ?d ?  ?- ?i ?n ?b ?o ?x return ?\C-n] 0 "%d"))


     (use-package notmuch
       :bind (("C-c m" . notmuch)
	      :map notmuch-search-mode-map
	      ("DEL" . my-notmuch-direct-archive)))
   #+END_SRC
