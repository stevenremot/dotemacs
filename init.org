...
** Configuration
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defgroup my-gitlab () "Gitlab tools" :group 'tools)

     (defcustom my-gitlab/repo-url ""
...
** Opening the issue at point
   ...
   Same system, for gitlab

   #+BEGIN_SRC emacs-lisp
     (defun my-gitlab/browse-issue (issue-number)
       "Visit the URL corresponding to the given ISSUE-NUMBER."
       (browse-url (format "%s/issues/%s" my-gitlab/repo-url issue-number)))

     (defun my-gitlab/browse-issue-at-point (issue-number)
       "Visit the URL corresponding to the ISSUE-NUMBER at point."
       (interactive (list (number-to-string (thing-at-point 'number))))
       (my-gitlab/browse-issue issue-number))

     (global-set-key (kbd "C-c g j") #'my-gitlab/browse-issue-at-point)
   #+END_SRC

** Opening the currently clocked-in issue

   #+BEGIN_SRC emacs-lisp
     (defun my-forge/get-current-issue ()
       "Return the issue number of the issue currently clocked in."
	 ...
	 (message "No issue currently clocked in")))

     (defun my-gitlab/browse-current-issue ()
       "Browse the gitlab issue currently clocked in."
       (interactive)
       (my-gitlab/browse-issue (my-forge/get-current-issue)))

     (global-set-key (kbd "C-c g J") #'my-gitlab/browse-current-issue)
   #+END_SRC
...
** Typescript
   ...
   Setup TIDE for extra IDE features. LSP would be better, but I had
   issues with the language servers I tried.

   #+BEGIN_SRC emacs-lisp
     (use-package tide
       :ensure
       :config (flycheck-add-next-checker 'typescript-tide 'typescript-tslint)
